...
### 4. Что такое data race и как её избежать в Go?


#### 🧠 Мнемоника: "МуКаАт"
Способы защиты от data race:
- **Му** — Mutex
- **Ка** — Каналы
- **Ат** — Atomic
...
DATA RACE - ПРОБЛЕМА:
┌──────────┐          ┌─────────┐          ┌──────────┐
│Goroutine1│─────────>│counter++│<─────────│Goroutine2│
│          │одновремен│ ГОНКА!  │одновремен│          │
└──────────┘          └─────────┘          └──────────┘
                           ↓
...
РЕШЕНИЕ 1: MUTEX
┌──────────┐          ┌─────────────────┐          ┌──────────┐
│Goroutine1│─────────>│ mu.Lock()       │          │Goroutine2│
│          │  получил │ counter++       │  ждёт... │  БЛОК    │
│          │  замок   │ mu.Unlock()     │          │          │
└──────────┘          └─────────────────┘          └──────────┘
                           ↓
                    ✅ Безопасно: по очереди


РЕШЕНИЕ 2: КАНАЛЫ
┌──────────┐          ┌─────────────────┐          ┌──────────┐
│Goroutine1│────────> │   ch <- data    │ <────────│Goroutine2│
│          │          │   БЕЗОПАСНО     │          │          │
└──────────┘          └─────────────────┘          └──────────┘
                           ↓
      ┌────────────────────┴────────────────────┐
      │        Worker Goroutine                 │
      │    for data := range ch { process() }   │
      └─────────────────────────────────────────┘
...
РЕШЕНИЕ 3: ATOMIC
┌──────────┐          ┌──────────────────────┐          ┌──────────┐
│Goroutine1│─────────>│atomic.AddInt64(&cnt,1)│<─────────│Goroutine2│
│          │  атомарно│   НА УРОВНЕ CPU!     │  атомарно│          │
└──────────┘          └──────────────────────┘          └──────────┘
                           ↓
                    ✅ Быстро и безопасно
```
...
### 5. Для чего используется пакет `context` и какие основные типы контекстов существуют?
...
### 6. Что такое модель GMP в планировщике Go?
...
### 7. Что такое sync.WaitGroup и как его правильно использовать?
...
### 8. Что такое sync.Once и когда его использовать?
...
### 9. Что такое deadlock и как он возникает в Go?
...
### 10. Что такое пакет sync/atomic и когда его использовать вместо мьютексов?
...
### 11. В чём разница между sync.Mutex и sync.RWMutex?
...
### 12. Что такое утечка горутин (goroutine leak) и как её обнаружить?
...
### 13. Что произойдёт при записи/чтении в nil-канал и в закрытый канал?
...
### 14. В чём разница между конкурентностью и параллелизмом?
...
### 15. Как правильно остановить горутину?
...
### 16. Почему нельзя использовать time.After в цикле select и как это приводит к утечке памяти?
...
### 17. Как ограничить количество одновременно работающих горутин?
...
### 18. Что такое sync.Cond и когда его использовать вместо каналов?
...
### 19. Что такое паттерны Fan-Out и Fan-In в Go?
...
### 20. Что такое zero value в Go и какие типы имеют какие значения по умолчанию?
...
### 21. В каком порядке выполняются defer и когда вычисляются их аргументы?
...
### 22. Как устроен слайс внутри и чем он отличается от массива?


#### 🧠 Мнемоника: "УкДлиЕм"
Слайс = Структура из 3 полей:
- **Ук** — Указатель на базовый массив
- **Дли** — Длина (len)
- **Ем** — Ёмкость (cap)

#### 📊 Визуальная схема
```
СЛАЙС - ЭТО ДЕСКРИПТОР:
┌─────────────────────────┐
│        СЛАЙС            │
│  ┌──────────────────┐   │
│  │ ptr (указатель)  │──────┐
...
ВАЖНО: Слайс и массив делят память!
s[0] = 99  →  arr теперь [1,99,3,4,5]
```
...
| **Тип** | `[5]int` | `[]int` |
| **Значение** | Копирует все элементы | Копирует дескриптор (24 байта) |
| **Сравнение** | `==` работает | `==` только с `nil` |
| **Рост** | Невозможен | `append()` |
| **В памяти** | Все данные | Указатель + len + cap |
...
### 23. Как устроена map в Go и что произойдёт при конкурентной записи?


#### 🧠 Мнемоника: "МапКонПан"
- **Мап** — Мапа
- **Кон** — Конкурентная запись
- **Пан** — Паника! (фатальная ошибка)

#### 📊 Визуальная схема
```
ПРОБЛЕМА: CONCURRENT MAP WRITES
┌──────────────┐                    ┌──────────────┐
...
│              │
│  mu.Lock()   │──────┐
│  m["key"]=1  │      │  Только один поток
│  mu.Unlock() │<─────┘  работает с map
└──────────────┘
...
│  │ Store(k, v)  │   │  ✅ Безопасно
│  │ Load(k)      │   │  ⚡ Оптимизирована для:
│  │ Delete(k)    │   │     - Много чтений
│  └──────────────┘   │     - Мало записей
└─────────────────────┘
...
РЕШЕНИЕ 3: CHANNELS
┌───────────┐     ┌────────────────┐     ┌───────────┐
│Goroutine 1│────>│ ch <- write{k,v}────>│  Worker   │
└───────────┘     └────────────────┘     │ (владелец)│
┌───────────┐     ┌────────────────┐     │   map)    │
│Goroutine 2│────>│ ch <- read{k}  ├────>│           │
└───────────┘     └────────────────┘     └───────────┘
                Один владелец — безопасно!
```
...
### 24. Что такое строка в Go и почему она иммутабельна?
...
### 25. Что такое embedding в Go и чем оно отличается от наследования?
...
### 26. Что такое пустая структура `struct{}` и когда её использовать?
...
### 27. В чём разница между pointer receiver и value receiver?
...
### 28. В чём разница между make и new?
...
### 29. Можно ли сравнивать структуры в Go и что такое ограничение comparable?
...
### 30. Почему порядок итерации по map не детерминирован?
...
### 31. Что такое type assertion и type switch? Когда возникает panic?
...
### 34. Как проверить на этапе компиляции, что тип реализует интерфейс?
...
### 35. Что такое теги структур (struct tags) и как они работают?
...
### 36. Что такое variadic functions и как передать слайс в такую функцию?
...
### 37. Как работает range и какие особенности при итерации по разным типам?
...
### 38. Как работает функция copy для слайсов?
...
### 39. В чём разница между nil-слайсом и пустым слайсом?
...
### 40. Что такое выравнивание структур (struct alignment) и как оно влияет на размер?
...
### 41. Что такое дженерики (generics) в Go и какие ограничения у type parameters?
...
### 42. Какие типы данных могут быть ключами map в Go?
...
### 43. В чём разница между стеком и кучей в Go и почему аллокация в куче дороже?
...
### 44. Как работает сборщик мусора в Go?


#### 🧠 Мнемоника: "ТриЦвет"
- **Три** — Три цвета (белый, серый, чёрный)
- **Цвет** — Цветовая маркировка объектов

#### 📊 Визуальная схема
```
GO GC: CONCURRENT MARK & SWEEP

ФАЗА 1: МАРКИРОВКА (Tricolor Algorithm)
┌─────────────────────────────────────────┐
│ Белый = кандидат на удаление            │
│ Серый = проверяется                     │
│ Чёрный = живой, проверен                │
└─────────────────────────────────────────┘

Шаг 1: Корни (стек, глобальные)
┌──────┐                ┌──────┐
│ЧЁРНЫЙ│───────────────>│СЕРЫЙ │
│Root  │                │ Obj  │
└──────┘                └──────┘
...
Шаг 3: Финал
┌──────┐    ┌──────┐    ┌──────┐    ┌──────┐
│ЧЁРНЫЙ│───>│ЧЁРНЫЙ│───>│ЧЁРНЫЙ│    │БЕЛЫЙ │← УДАЛИТЬ!
└──────┘    └──────┘    └──────┘    └──────┘


ФАЗА 2: ОЧИСТКА (Sweep)
Память:  [USED][FREE][USED][FREE][USED]
            ↓     ↓     ↓     ↓     ↓
Sweep:  [USED][████][USED][████][USED]
                ↑            ↑
            Освобождено!
...
TIMELINE GC В GO:
├────┬────────┬────┬─────────────┬────┬───>
│ STW│ Mark   │STW │   Sweep     │STW │
│ 1  │(конк.) │ 2  │(конкурентно)│ 3  │
└────┴────────┴────┴─────────────┴────┘
 <0.5ms  ~ms   <0.5ms    ~ms     <0.5ms

STW = Stop-The-World (очень короткие!)
```
...
### 45. Что такое pprof и как его использовать для профилирования Go-приложения?
...
### 46. В чём разница между стеком и кучей в Go и почему аллокация в куче дороже?
...
### 47. Что такое sync.Pool и когда его использовать?
...
### 48. Как устроен интерфейс внутри и чем отличается iface от eface?
...
### 49. Как писать бенчмарки в Go и что такое b.N?
...
### 50. Зачем нужен strings.Builder и почему конкатенация через + неэффективна в цикле?
...
### 51. В чём разница между nil interface и interface с nil-значением внутри?
...
### 52. Как растёт capacity слайса при append и почему?
...
### 53. Когда использовать panic/recover и почему recover работает только в defer?
...
### 54. Как работает wrapping ошибок и зачем нужны errors.Is и errors.As?
...
### 55. Что такое замыкание (closure) и какая типичная ошибка связана с циклами?
...
### 56. Что такое функция init() и в каком порядке она вызывается?
...
### 57. Что такое рефлексия (reflection) в Go и когда её использовать?
...
### 58. Можно ли вызвать метод на nil-указателе и когда это работает?
...
### 59. Как defer взаимодействует с именованными возвращаемыми значениями?
...
### 60. Как работает тестирование в Go и что такое table-driven tests?
...
### 61. Что такое build tags (build constraints) и для чего они используются?
...
### 64. Как реализовать graceful shutdown HTTP-сервера в Go?
...
### 65. Как работает http.ListenAndServe и ServeMux внутри?
...
### 66. Что такое идемпотентность HTTP-методов и какие методы являются идемпотентными?
...
### 67. Чем отличаются HTTP-методы PUT и PATCH?
...
### 68. Какие основные HTTP-статус-коды должен знать backend-разработчик?
...
### 69. Что такое CORS и зачем он нужен?
...
### 70. Что такое REST и какие принципы REST-архитектуры?
...
### 71. В чём основные отличия HTTP/2 от HTTP/1.1?
...
### 72. В чём разница между JWT и сессионной аутентификацией?
...
### 73. Что такое gRPC и чем он отличается от REST?
...
### 74. Какие способы версионирования REST API существуют и какой предпочтительнее?
...
### 75. Что такое HATEOAS и почему его редко используют на практике?
...
### 76. Что такое Idempotency Key и зачем он нужен в API?
...
### 77. Что такое singleflight и когда его использовать?
...
### 78. Чем микросервисная архитектура отличается от монолита?
...
### 79. Что такое Circuit Breaker и зачем он нужен в микросервисах?
...
### 80. Что такое connection pooling и зачем он нужен?
...
### 81. Что такое CAP-теорема и почему нельзя получить все три свойства одновременно?
...
### 82. Что такое API Gateway и зачем он нужен в микросервисах?
...
### 83. Какие алгоритмы rate limiting существуют и чем они отличаются?
...
### 85. Что такое Dead Letter Queue и зачем она нужна?
...
### 86. Что такое reverse proxy и зачем он нужен?
...
### 87. Какие уровни изоляции транзакций существуют в базах данных?
...
### 88. Что такое N+1 проблема в базах данных и как её решить?
...
### 89. Что такое индекс в базе данных и когда его НЕ стоит использовать?
...
### 90. Что такое ACID в контексте баз данных?
...
### 91. Чем отличается синхронная репликация от асинхронной?
...
### 93. Какие типы JOIN существуют в SQL и чем они отличаются?
...
### 94. Чем отличается оптимистичная блокировка от пессимистичной?
...
### 95. Чем Docker-контейнер отличается от виртуальной машины?
...
### 96. Что такое multi-stage build в Docker и зачем он нужен для Go-приложений?
...
### 97. Чем отличаются сигналы SIGTERM и SIGKILL в Linux?
...
### 98. Чем отличается TCP от UDP и что такое TCP handshake?
...
### 99. Что происходит, когда вводишь URL в браузер и нажимаешь Enter?
...
### 100. Что такое OWASP Top 10 и какие уязвимости туда входят?
